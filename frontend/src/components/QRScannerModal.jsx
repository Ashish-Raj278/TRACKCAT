...
export default function QRScannerModal({ isOpen, onClose, assets = [], onSelectAssetForCheckout, onSelectAssetForCheckin }) {
  ...
  useEffect(() => {
    if (assets.length > 0) setAllAssets(assets);
    else getAssets().then(d => { if (d && Array.isArray(d)) setAllAssets(d); }).catch(() => {});
  }, [assets]);
  ...
  const handleLookup = useCallback((code) => {
    ...
    if (!clean) { setError('Please enter an equipment ID.'); return; }
    setError(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl my-4">
        {/* Close */}
        <button
          onClick={async () => { await stopCamera(); setScannedAsset(null); setError(null); onClose(); }}
